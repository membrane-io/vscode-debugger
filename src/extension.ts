...
import { QuickJSDebugSession } from "./membraneDebug";
import * as Net from "net";
import { getCurrentProgramName, getCurrentProgramUri } from "./util";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.debug.registerDebugConfigurationProvider("membrane", provider)
  );
  ...
  context.subscriptions.push(
    vscode.debug.registerDebugAdapterDescriptorFactory("membrane", factory)
  );
  context.subscriptions.push(factory);

  context.subscriptions.push(
    vscode.commands.registerCommand(
      "membrane.debugCurrentProgram",
      async () => {
        const uri = await getCurrentProgramUri();
        const program = await getCurrentProgramName();
        if (uri) {
